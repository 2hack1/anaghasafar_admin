...
@Component({
  selector: 'app-destination-hotel-booking-show',
  imports: [FormsModule,NotifierModule,CommonModule],
  templateUrl: './destination-hotel-booking-show.html',
  styleUrl: './destination-hotel-booking-show.scss'
})
export class DestinationHotelBookingShow implements OnInit {
...
bokingdata:any;
 getbookingsdata(){
  this.userservice.getpaid().subscribe((res:any)=>{  //  good work
    this.bokingdata=res;
  })
 }
...
getbooking(){
  ...
  this.userservice.getunpaid().subscribe({
    next: (res: any) => {
