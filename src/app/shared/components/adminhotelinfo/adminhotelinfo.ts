...
@Component({
  selector: 'app-adminhotelinfo',
  imports: [CommonModule,FormsModule,NotifierModule],
  templateUrl: './adminhotelinfo.html',
  styleUrl: './adminhotelinfo.scss'
})
export class Adminhotelinfo {
...
ngOnInit(): void {
  ...
  // this.userservice.gethotelprofile().subscribe((res:any)=>{
...
getbooking(){
  ...
  this.userservice.gethotelprofile().subscribe({
    next: (res: any) => {
...
applyFilters() {
  ...
  if (this.searchQuery.trim()) {
    ...
    data = data.filter(b =>
      ...
      b.hotel_vendor_id.toString().includes(q) ||
